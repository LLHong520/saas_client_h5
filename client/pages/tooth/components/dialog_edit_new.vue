      ...
      <div class="d-dialog-container" v-show="isShow" @click.stop>
        ...
        <div @click.stop>
          <div class="hospital-info">
            <div class="header">
              <div class="logo">
                <img :src="options.icon" alt="" :title="options.name">
              </div>
              <div class="name">
                {{options.name}}
              </div>
            ...
            <div class="recommend">
              <p class="recommend-title">{{options.recommendTitle}}</p>
              <ul class="goods">
                <li v-for="(v,k) in options.services" :key="k">
                  <span class="current">{{v.currentText}}</span>
                  <span class="old">{{v.oldText}}</span>
                </li>
              </ul>
            </div>
          </div>
          ...
          <div class="content-form">
            ...
            <div class="input-line encrypt input-form-phone">
              ...
              <input v-model="params.phone" ref="encrypt" type="number" placeholder="请输入手机号码" oninput="if(value.length>11)value=value.slice(0,11)" pattern="[0-9]*">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getSubmitParams() {
        let { type, id,name} = this.options;
        // let { query } = this.$route;

        return {
          name: this.params.name,
          phone: this.params.phone,
          hospitalId: `${type}_${id}_${name}`,
