...
<script>
  import hospitalList from "../js/hospitalsNew";
  import { PROJECT_ENV } from "~/config";
  import pageDataCollectMixin from "../js/pageDataCollectMixin";

  export default {
    name: "hospitalListNew",
    components: {
      dialogEdit:()=> import("./dialog_edit_new")
    },
    props: {
      type:{
        type:String,
    ...
    methods: {
      selectHospital(v){
        ...
        //打点
        this.recordPage({
          questionNo:`${v.id}-${v.name}`,
          operationType:"hospital_card"
        });
      },
      successDialogEdit(){
